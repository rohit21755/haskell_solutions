...
sums i = sumHelper 1 i
  where
    sumHelper acc 1 = [acc]
    sumHelper acc n = acc : sumHelper (acc + n) (n - 1)
...
sumsOf :: [Int] -> [Int]
sumsOf [] = []
sumsOf (x:xs) = x : sumsOfPartialSum x xs
 where
    sumsOfPartialSum _ [] = []
    sumsOfPartialSum acc (y:ys) = acc + y : sumsOfPartialSum (acc + y) ys
