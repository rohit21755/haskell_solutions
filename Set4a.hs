...
distinct xs = null (filter ((>1) . length) (group (sort xs)))
...
longest :: Ord a => [[a]] -> [a]
longest [] = []
longest (x:xs) = longestAux x xs
 where
    longestAux l [] = l
    longestAux l (x:xs)
        | length l > length x || (length l == length x && head l < head x) = longestAux l xs
        | otherwise = longestAux x xs
...
freqs xs = foldr updateFreq Map.empty xs
  where
    ...
    updateFreq key freqMap = Map.alter (\maybeFreq -> Just (maybeFreqValue maybeFreq + 1)) key freqMap

    maybeFreqValue :: Maybe Int -> Int
    maybeFreqValue Nothing  = 0
    maybeFreqValue (Just n) = n
...
transfer from to amount bank
  ...
  | otherwise = Map.adjust (\bal -> bal - amount) from $ Map.adjust (+amount) to bank
  where
