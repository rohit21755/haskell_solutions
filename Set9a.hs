...
workload nExercises hoursPerExercise = 
    ...
    in if totalHours > 100 then "Holy moly!"
...
echo :: String -> String
echo [] = ""
echo (x:xs) = x : echoHelper xs ++ ", "
    where
        echoHelper [] = ""
        echoHelper (y:ys) = y : echoHelper ys
...
-- Helper function to check the validity of a single banknote serial number
isValid :: String -> Bool
isValid serial =
    let third = serial !! 2
        fifth = serial !! 4
...
-- Function to count the number of valid banknote serial numbers
...
compose f g = [(k, v) | (k, v1) <- f, (v1', v) <- g, v1 == v1']
...
permute indices xs = [xs !! i | i <- indices]
