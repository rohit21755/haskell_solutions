...
isSorted xs = property $ sort xs == xs
...
outputInInput input output = conjoin [counter x == n | (x, n) <- output]
  where
    counter x = length $ filter (== x) input
...
frequenciesProp freq (NonEmpty input) = sumIsLengthProp .&&. inputInOutputProp .&&. outputInInputProp
  where
    output = freq input
    sumIsLengthProp = sumIsLength input output
    inputInOutputProp = inputInOutput input output
    outputInInputProp = outputInInput input output
...
genList = do
  len <- choose (3, 5)
  elements <- vectorOf len (choose (0, 10))
