...
firstHalf str
  | even len  = take (len `div` 2) str
  | otherwise = take ((len + 1) `div` 2) str
  where
    len = length str
...
joinToLength len strings =
  [concat [x, y] | x <- strings, y <- strings, length (x ++ y) == len]
...
interpreter commands = interpret commands (0, 0)
  where
    interpret :: [String] -> Position -> [String]
    interpret [] _ = []
    interpret (cmd:rest) pos =
      case cmd of
        "up"     -> interpret rest (moveUp pos)
        "down"   -> interpret rest (moveDown pos)
        "left"   -> interpret rest (moveLeft pos)
        "right"  -> interpret rest (moveRight pos)
        "printX" -> show (fst pos) : interpret rest pos
        "printY" -> show (snd pos) : interpret rest pos
        _        -> interpret rest pos
