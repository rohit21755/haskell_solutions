...
count x = do
  state <- get
  let updatedState = updateState x state
  put updatedState

updateState :: Eq a => a -> [(a,Int)] -> [(a,Int)]
updateState x [] = [(x,1)]
updateState x ((y,n):ys)
  | x == y    = (y,n+1) : ys
  | otherwise = (y,n) : updateState x ys
