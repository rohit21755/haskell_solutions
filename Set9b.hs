...
prettyPrint size queens = unlines [ [if (r, c) `elem` queens then 'Q' else '.' | c <- [1..size]] | r <- [1..size] ]
...
sameRow (_, col1) (_, col2) = col1 == col2

sameCol :: Coord -> Coord -> Bool
sameCol (row1, _) (row2, _) = row1 == row2

sameDiag :: Coord -> Coord -> Bool
sameDiag (row1, col1) (row2, col2) = abs (row1 - row2) == abs (col1 - col2)

sameAntidiag :: Coord -> Coord -> Bool
sameAntidiag (row1, col1) (row2, col2) = row1 + col1 == row2 + col2
...
danger coord queens = any (\queen -> sameRow coord queen || sameCol coord queen || sameDiag coord queen || sameAntidiag coord queen) queens
...
prettyPrint2 size queens = unlines [ [if (r, c) `elem` queens then 'Q' else if danger (r, c) queens then '#' else '.' | c <- [1..size]] | r <- [1..size] ]
...
fixFirst n [] = Just []  -- Empty stack, nothing to fix
fixFirst n ((r, c):qs)
  | c > n = Nothing  -- Queen already outside the board
  | danger (r, c) qs = case find (\col -> not $ danger (r, col) qs) [c+1..n] of
                          Just safeCol -> Just ((r, safeCol):qs)
                          Nothing      -> Nothing
...
continue [] = error "Cannot continue on an empty stack"
continue ((r, c):qs) = (nextRow (r, 1)) : ((nextRow (r, 1)):qs)

backtrack :: Stack -> Stack
backtrack [] = error "Cannot backtrack on an empty stack"
backtrack [_] = error "Cannot backtrack when there's only one queen on the stack"
backtrack (_:qs) = qs
...
finish size stack
  | length stack == size + 1 = stack  -- Complete solution found
  | otherwise = case fixFirst size stack of
                  Just newStack -> finish size (continue newStack)
                  Nothing       -> backtrack stack
