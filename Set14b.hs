...
-- Database
import Database.SQLite.Simple (open,execute,execute_,query,query_,Connection,Query(..), Only (Only))
...
balance conn account = do
    ...
    return (sum (map (\(Only amount) -> amount) amounts))
...
parseCommand [cmd, account, amount]
    | cmd == T.pack "deposit" = Deposit <$> Just account <*> parseInt amount
...
server db request respond = do
    ...
    case command of
        Nothing -> respond $ responseLBS status400 [] (encodeResponse (T.pack "Bad Request"))
        Just cmd -> do
