...
paintSolid color shape (Picture base) = Picture f
  where f coord | contains shape (getX coord) (getY coord) = color
...
instance Transform Flip where
  apply FlipX = flipXY
  apply FlipY = flipXY . flipXY
  apply FlipXY = flipXY . flipXY . flipXY
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture f) = Picture (\(Coord x y) -> avgColor (f (Coord x y)) (neighbors x y))
    where avgColor c1 cs = foldl combineColors c1 cs
          neighbors x y = [f (Coord x' y') | x' <- [x-1..x+1], y' <- [y-1..y+1], x /= x' || y /= y']
          combineColors (Color r1 g1 b1) (Color r2 g2 b2) = Color ((r1 + r2) `div` 2) ((g1 + g2) `div` 2) ((b1 + b2) `div` 2)
------------------------------------------------------------------------------
