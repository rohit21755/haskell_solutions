...
isOrdered tree = isOrderedHelper tree Nothing Nothing

isOrderedHelper :: Ord a => Tree a -> Maybe a -> Maybe a -> Bool
isOrderedHelper Empty _ _ = True
isOrderedHelper (Node value left right) minVal maxVal =
  maybe True (value >) minVal &&
  maybe True (value <) maxVal &&
  isOrderedHelper left minVal (Just value) &&
  isOrderedHelper right (Just value) maxVal
...
set [] _ tree = tree
set (s:ss) val (Node _ left right)
    | s == StepL = Node val (set ss val left) right
    | s == StepR = Node val left (set ss val right)
...
search target tree = searchHelper target tree []

searchHelper :: Eq a => a -> Tree a -> [Step] -> Maybe [Step]
searchHelper _ Empty _ = Nothing
searchHelper target (Node value left right) path
  | target == value = Just path
  | otherwise =
    case searchHelper target left (path ++ [StepL]) of
      Just result -> Just result
      Nothing -> searchHelper target right (path ++ [StepR])
