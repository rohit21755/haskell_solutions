...
instance Ord Country where
  ...
  Finland <= other = compare Finland other /= GT
  Norway <= other  = compare Norway other /= GT
  Switzerland <= other = compare Switzerland other /= GT
...
class Price2 a where
  price2 :: a -> Int

data Egg2 = ChickenEgg2 | ChocolateEgg2
  deriving Show

data Milk2 = Milk2 Int -- amount in litres
  deriving Show

instance Price2 Egg2 where
  price2 ChickenEgg2 = 20
  price2 ChocolateEgg2 = 30

instance Price2 Milk2 where
  price2 (Milk2 liters) = 15 * liters
...
simplify (RationalNumber a b) = RationalNumber (a `div` commonFactor) (b `div` commonFactor)
  where
