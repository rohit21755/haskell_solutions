...
reverseNonEmpty (x :| xs) = go (x :| []) xs
  where
    go acc [] = acc
    go (h :| t) (elem:rest) = go (elem :| (h : t)) rest
...
passwordLength :: String -> PasswordRequirement -> Bool
passwordLength pass (MinimumLength len) = length pass >= len

containsSome :: String -> PasswordRequirement -> Bool
containsSome pass (ContainsSome chars) = any (`elem` chars) pass

doesNotContain :: String -> PasswordRequirement -> Bool
doesNotContain pass (DoesNotContain chars) = all (`notElem` chars) pass

passwordAllowed :: String -> PasswordRequirement -> Bool
passwordAllowed pass req = case req of
 MinimumLength len -> passwordLength pass req
 ContainsSome chars -> containsSome pass req
 DoesNotContain chars -> doesNotContain pass req
 And req1 req2 -> passwordAllowed pass req1 && passwordAllowed pass req2
 Or req1 req2 -> passwordAllowed pass req1 || passwordAllowed pass req2
