...
statements xs ys = liftA2 combine xs ys
  where
    ...
    combine x y = unwords [x, "is", y] ++ " " ++ unwords [x, "is not", y]
...
validateDiv x y = if y /= 0 then pure (x `div` y) else invalid "Division by zero!"
...
validateAddress streetName streetNumber postCode =
  Address <$> validateStreet streetName <*> validateStreetNumber streetNumber <*> validatePostcode postCode
  where
    validateStreet :: String -> Validation String
    validateStreet street =
      if length street <= 20 then pure street else invalid "Invalid street name"

    validateStreetNumber :: String -> Validation String
    validateStreetNumber streetNum =
      if all isDigit streetNum then pure streetNum else invalid "Invalid street number"

    validatePostcode :: String -> Validation String
    validatePostcode postcode =
      if length postcode == 5 && all isDigit postcode then pure postcode else invalid "Invalid postcode"
  ...
  case readMaybe str of
    ...
    Nothing ->
      case str of
        ...
        _       -> invalid "Not a Bool" <|> invalid "Not an Int"
...
normalizePhone str =
  ...
  in if length stripped > 10
       ...
       else if all isDigit stripped
              then pure stripped
              else invalidChars stripped
  where
    invalidChars :: String -> Validation String
    invalidChars s = traverse (\c -> if isDigit c then pure c else invalid $ "Invalid character: " ++ [c]) s
...
myFmap f x = myLiftA2 (\g val -> g val) (myPure f) x
...
tryAll _ [] = empty  -- Base case: Empty list, return empty result
tryAll f (x:xs) = f x <|> tryAll f xs  -- Apply f to the head of the list, combine result with recursive call on the tail
